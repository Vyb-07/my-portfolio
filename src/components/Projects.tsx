        ...
        <section id="projects">
            ...
            <div className="grid gap-6 md:grid-cols-2 max-w-4xl mx-auto">
                {projects.map((project, idx) => (
                    <Card key={idx} className="group hover:bg-neo-primary dark:hover:bg-blue-600 hover:text-white transition-colors">
                        <div className="flex justify-between items-start mb-4">
                            <h3 className="text-2xl font-bold dark:text-white">{project.title}</h3>
                            <div className="flex gap-2">
                                <a href={project.github} className="p-2 border-2 border-neo-border dark:border-white rounded-md hover:bg-white hover:text-black dark:hover:bg-slate-700 dark:hover:text-white transition-colors dark:text-white"><Github size={20} /></a>
                                <a href={project.link} className="p-2 border-2 border-neo-border dark:border-white rounded-md hover:bg-white hover:text-black dark:hover:bg-slate-700 dark:hover:text-white transition-colors dark:text-white"><ExternalLink size={20} /></a>
                            </div>
                        </div>
                        <p className="mb-6 font-medium text-slate-700 dark:text-slate-300 group-hover:text-white/90">{project.desc}</p>
                        <div className="flex flex-wrap gap-2">
                            {project.tech.map((t, i) => (
                                <span key={i} className="px-3 py-1 text-sm font-bold border-2 border-neo-border dark:border-white rounded-full bg-white dark:bg-slate-700 text-black dark:text-white shadow-neo-sm dark:shadow-[2px_2px_0px_0px_#ffffff] group-hover:shadow-none transition-all">
                                    {t}
                                </span>
                            ))}
                        </div>
